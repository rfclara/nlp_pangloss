...
def save_audio_chunk(chunk, output_dir, audio_file, segment_id, format="wav"):
    """Saves an audio chunk to the output directory."""
    output_dir.mkdir(parents=True, exist_ok=True)
    chunk_path = output_dir / f"{Path(audio_file).stem}_s{segment_id}.{format}"
...
def process_segment(segment, audio_file, output_dir, segment_id):
    """Processes a single XML segment and saves the corresponding audio chunk."""
    ...
    chunk_path = save_audio_chunk(chunk, output_dir, audio_file, segment_id)
...
def parse_xml_and_extract_audio(xml_file, output_dir):
    """Main function to parse XML, process segments, and save audio chunks and annotations."""
    ...
    for segment in root.findall("S"):
        try:
            result = process_segment(segment, audio_file, output_dir, segment_id)
            rows.append(result)
    ...
    if not rows:
        raise ValueError("No valid segments were processed.")
